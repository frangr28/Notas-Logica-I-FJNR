...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!blue,title=\textbf{Lema}.]
Sea \(\Gamma\subseteq\overline{L}\).
\(\Gamma\) es inconsistente si y sólo si existe \(\varphi\in\overline{L}\) tal que \(\Gamma\not\vdash\varphi\).
\end{tcolorbox}

\begin{tcolorbox}
\textit{Prueba}. 
\tcblower
\(\boxed{\Rightarrow}\) Supongamos que para toda \(\varphi\in\overline{L}\) se tiene que \(\Gamma\vdash\varphi\). En particular \(\Gamma\vdash P_0\) y \(\Gamma\vdash(\neg P_0)\), entonces \(\Gamma\) es inconsistente.

\(\boxed{\Leftarrow}\) Supongamos que \(\Gamma\) es inconsistente, es decir, existe \(\alpha\in\overline{L}\) tal que \(\Gamma\vdash\alpha\) y \(\Gamma\vdash\neg\alpha\) \(\dots(ii)\).

Sea \(\varphi\in\overline{L}\). P.D. \(\Gamma\vdash\varphi\).

Usando la definición de deducción y monotonía se tiene que \(\Gamma\vdash\)

Por otro lado, igual por monotonía y definición de deducción, 

Análogamente, 

Por \textsf{MMP}(\(i, \star\star\)), \(\Gamma\vdash(\neg\varphi\to\alpha)\).
Análogamente \(\Gamma\vdash(\neg\varphi\to(\neg\alpha))\), \textsf{MMP}(\(ii, \star\star\))

De \textsf{MMP}(\(iii, \star\star\star\star\)), \(\Gamma\vdash((\neg\varphi)\to\alpha)\to\varphi\)

Usando \textsf{MMP}(\(iv, \star\star\star\)), \(\Gamma\vdash\varphi\).
\end{tcolorbox}
